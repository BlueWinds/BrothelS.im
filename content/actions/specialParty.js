define(['messages/messages'], function(Message) {
  return {
    ...
    results: [
      {
        image: ['<%= mission.people[0].image %>', 'content/missions/festival.jpg', '<%= girl.image(mission.people[0].sex[0]) %>'],
        message: [
          "<%= girl.name %> arrived at <%= mission.people[0].name %>'s in early, wearing her finest clothing. She was met in the common room by a servant - the master wasn't ready quite yet, having stayed up a little too late last night. The servant apologized for the commonness of the surroundings - at home, <%= mission.people[0].name %> had a fine mansion, but not planning on staying in this city long, had decided to make do by renting a set of rooms. <%= girl.name %> nodded and smiled - in her eyes, the inn was quite a fine one, nothing to be apologizing over at all.<br><br><%= mission.people[0].name %> finally arrived and took her hand.",
          "The main street is crowded to the brim with party-goers, lit from above by strings of paper lanterns and by a seemingly endless stream of candles floating on tiny rafts down the river, each one carrying the sorrows or regrets of someone in the city out to sea. Initially the party is merely loud and exciting - <%= mission.people[0].name %> and <%= girl.name %> wander the streets, and <%= mission.people[0].name %> buys an endless stream of whatever nick-nacks or delicacies catches either one of their interests.<br><br>As the evening goes on and drink begins to set in, clothing begins to dwindle - first a set of young men missing their shirts, then a dancing circle of women in nothing but their underwear. Now when did <%= girl.name %> loose her shirt? She can't quite remember - probably when someone spilled a beer. <%= mission.people[0].name %> remains a perfect gentleman even as she looses steadily more clothing to \"accidents\" - and finally, he's happily parading her around the streets totally naked.",
