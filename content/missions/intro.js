...
Missions.libidoAndExperience = {
  display: {
    ...
    weight: -1,
    text: "<blockquote>Preparations almost complete! I'm super psyched - I haven't had a good fucking in way too long. Let's get out on the street and start looking for customers.</blockquote>Before you do that, there's one last thing you need to do - make sure Kirino is lusty enough to server as a street whore. You decide to test this out by subjecting her to some <strong>Orgasm Denial</strong> to raise her <<- __('libido') >>. Some actions, like this one, have multiple options - once you assign a girl to them, you can change which option you'd like to use by hovering over the label and clicking on one of them. For now, make Kirino with with Kirino on <<- __('hard') >>.<br><br><em>The last pair of stats for girls are <<- __('libido') >> and <<- __('experience') >>, and unlike other stats, these are broken up into four categories - <<- Girl.sex._toString('noun') >>. <<- __('libido') >> represents her level of lust, nymphomania and general desire for a specific type of sex. A <<- __('libido', 'adj') >> is significantly less likely to refuse sex of that type, while <<- __('experience') >> is a measure of both how often she's had sex and how good she is at pleasuring her partners. Experience will rise on its own whenever a girl has sex of that type. Both of these stats are very important in determining how much a customer pays for an encounter.</em>"
  },
  end: {
    girl: {
      min: { hardLibido: 61 }
